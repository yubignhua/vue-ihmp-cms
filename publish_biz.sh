...
# biz
_DEFAULT_TEST_BUSINESS_DIR=$TARGET_DOMAIN'/projects/biztest/chunyu-admin/data/'
# 部署目标目录
...
echo "******************3.同步资源到远程目标机器*********************"
# 同步静态资源
rsync -avP --delete ./dist/static/vue/vue_chunyu_admin_pc/ $TARGET_DIR'/static/vue/vue_chunyu_admin_pc' || { echo "command failed"; exit 1; }
# 同步index.html
rsync -avP --delete ./dist/index.html $TARGET_DIR'/templates/vue_chunyu_admin_pc' || { echo "command failed"; exit 1; }
echo "*****************************同步完成*************************"
